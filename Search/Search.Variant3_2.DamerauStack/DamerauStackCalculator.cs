...
/// Uses a SINGLE flat stackalloc buffer of 3×(n+1) integers.
/// Row indices are rotated by reassigning integer offset variables — no data movement.
///
/// FAST PATH (n ≤ 256): stackalloc — zero heap allocation.
/// SAFE FALLBACK (n > 256): ArrayPool.Shared.Rent() — no GC.
/// </summary>
internal static class DamerauStackCalculator
{
    ...
    private static int ComputeCore(
        ...
        Span<int> buffer)
    {
        // Row offset indices into the flat buffer
        int r0 = 0;           // row prev2 (i-2)
        int r1 = rowLen;      // row prev  (i-1)
        int r2 = 2 * rowLen;  // row curr  (i)

        // Row 0: dp[0][j] = j
        for (int j = 0; j <= n; j++) buffer[r0 + j] = j;

        // Row 1: no transpositions
        buffer[r1] = 1;
        for (int j = 1; j <= n; j++)
        {
            int cost = (source[0] == target[j - 1]) ? 0 : 1;
            buffer[r1 + j] = Math.Min(
                Math.Min(buffer[r1 + j - 1] + 1,   // insert
                         buffer[r0 + j]     + 1),  // delete
                         buffer[r0 + j - 1] + cost  // substitute
            );
        }
        ...
        for (int i = 2; i <= m; i++)
        {
            ...
            for (int j = 1; j <= n; j++)
            {
                ...
                buffer[r2 + j] = Math.Min(
                    Math.Min(buffer[r2 + j - 1] + 1,   // insert
                             buffer[r1 + j]     + 1),  // delete
                             buffer[r1 + j - 1] + cost  // substitute
                );
            ...
            // 3-way rotation of row offsets (ZERO data movement)
            int tmp = r0;
            r0 = r1;
            r1 = r2;
            r2 = tmp;
        ...
        // After last iteration r1 holds the final row
        return buffer[r1 + n];
    }
}
